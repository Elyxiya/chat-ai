...
export class AuthService {
  ...
  async register(dto: RegisterDto): Promise<AuthTokens> {
    ...
    // #region debug log
    fetch('http://127.0.0.1:7327/ingest/804a4ea0-edf2-4cdf-8542-0c7db0a68a39',{method:'POST',headers:{'Content-Type':'application/json','X-Debug-Session-Id':'d7dd50'},body:JSON.stringify({sessionId:'d7dd50',location:'auth.service.ts:43',message:'register check',data:{existing:!!existing,existingUserId:existing?.id,existingUsername:existing?.username,existingEmail:existing?.email,registerUsername:dto.username,registerEmail:dto.email},timestamp:Date.now()})}).catch(()=>{});
    // #endregion
    if (existing) {
  ...
  private async generateTokens(user: UserPayload): Promise<AuthTokens> {
    ...
    // #region debug log
    fetch('http://127.0.0.1:7327/ingest/804a4ea0-edf2-4cdf-8542-0c7db0a68a39',{method:'POST',headers:{'Content-Type':'application/json','X-Debug-Session-Id':'d7dd50'},body:JSON.stringify({sessionId:'d7dd50',location:'auth.service.ts:245',message:'generateTokens',data:{jwtSecretEnv:process.env.JWT_SECRET?.substring(0,20),userId:user.id},timestamp:Date.now()})}).catch(()=>{});
    // #endregion
