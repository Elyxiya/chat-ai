    ...
    try {
      ...
      if (isLogin) {
        if (status === 401) {
          ...
          } else if (msg.includes('incorrect') || msg.includes('wrong') || msg.includes('密码错误') || msg.includes('不正确')) {
            setError(t('auth.errorWrongPassword'));
          } else {
            setError(rawMessage || t('auth.errorUnknown'));
          }
        } else if (status === 429) {
        ...
        if (status === 409) {
          ...
          if (msg.includes('password') && (msg.includes('must contain') || msg.includes('too weak') || msg.includes('强度'))) {
            setError(t('auth.errorWeakPassword'));
          } else if (msg.includes('username') && (msg.includes('too short') || msg.includes('至少'))) {
            setError(t('auth.errorUsernameTooShort'));
          } else if (msg.includes('email') && (msg.includes('must be an email') || msg.includes('isemail') || msg.includes('valid'))) {
