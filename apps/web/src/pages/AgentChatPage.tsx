    ...
    <>
      ...
      <header className="h-14 px-4 border-b border-border flex items-center gap-3 bg-surface">
        ...
        <div className="flex-1">
          <h2 className="font-semibold text-sm">{t('agent.title')}</h2>
          <p className="text-xs text-text-secondary">
            {isStreaming ? t('agent.generating') : 'Powered by DeepSeek V3 + R1'}
          </p>
        </div>
        ...
        <div className="flex gap-1 bg-bg rounded-lg p-1">
          {(['react', 'planner', 'reasoner'] as const).map((m) => (
            ...
            >
              {m === 'react' ? t('agent.modeReact') : m === 'planner' ? t('agent.modePlanner') : t('agent.modeReasoner')}
            </button>
          ))}
        </div>
        ...
        {/* Speed control */}
        <div className="flex items-center gap-1.5 group relative" title={t('agent.typingSpeed') || `Typing speed: ${typingSpeed}ms`}>
        ...
        <div className="flex-1 flex flex-col overflow-hidden">
            ...
            {messages.length === 0 && !isStreaming && (
              <div className="h-full flex items-center justify-center">
                <div className="text-center max-w-md">
                  ...
                  <h3 className="text-lg font-semibold mb-2">{t('agent.title')}</h3>
                  <p className="text-text-secondary text-sm">
                    Powered by DeepSeek V3 + R1. {t('agent.welcomeDesc') || 'Try asking complex questions, planning tasks, or searching the knowledge base.'}
                  </p>
          ...
          <div className="p-4 border-t border-border bg-surface">
            ...
            <p className="text-xs text-text-secondary mt-2">
              {t('settings.title')}: {mode === 'react' ? t('agent.modeReact') : mode === 'planner' ? t('agent.modePlanner') : t('agent.modeReasoner')}
            </p>
          </div>
        </div>
            ...
            {showThinking && (
              <div className="mb-6">
                <div className="flex items-center gap-2 mb-3">
                  <h3 className="text-sm font-semibold">{t('agent.thinkingChain')}</h3>
                  {isStreaming && (
